...
def test_replace_chain_not_long_enough(blockchain_three_blocks):
    blockchain = Blockchain()
    with pytest.raises(Exception, match='Can not replace due to the shorter chain than local.'):
...
def test_replace_chain_bad_chain(blockchain_three_blocks):
    ...
    with pytest.raises(Exception, match='Can not replace due to invalid chain: '):
        blockchain.replace_chain(blockchain_three_blocks.chain)
...
def test_is_valid_chain_multiple_rewards(blockchain_three_blocks):
...
def test_is_valid_transaction_chain_bad_historic_balance(blockchain_three_blocks):
    wallet = Wallet()
    bad_transaction = Transaction(wallet, 'recipinet', 1)
